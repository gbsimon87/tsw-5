  ...
  try {
    ...
    for (const league of leagues) {
      ...
      for (let i = 0; i < config.gamesPerLeague; i++) {
        ...
        // Initialize playerStats with all stat categories for all players
        const playerStatsMap = {};
        for (const member of allPlayers) {
          const playerId = member.player.toString();
        ...
        // Calculate team scores based on scoringRules
        ...
        const game = new Game({
          ...
          playerStats: isCompleted ? playerStats : [],
          playByPlay: isCompleted ? playByPlay : [],
          isCompleted,
          matchType: 'league',
          eventType: 'regular',
          gameDuration: league.settings.periodDuration *
            (league.settings.periodType === 'quarters' ? 4 :
             league.settings.periodType === 'periods' ? 3 : 2),
        });
